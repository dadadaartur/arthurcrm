...
export default async function handler(req, res) {
  ...
  const { actionType, userId, userIds, title, reason, deadline, rewardKarma, testId, trainingNote } = req.body || {}
  const targets = userIds && Array.isArray(userIds) && userIds.length ? userIds : userId ? [userId] : []
  if (!targets.length || !title?.trim()) return res.status(400).json({ error: 'Укажите сотрудника(ов) и название' })
  ...
  for (const uid of targets) {
    ...
    if (actionType === 'task') {
      const { data: task, error } = await a.from('tasks').insert({
        ...
        reward_karma: Math.max(1, Number(rewardKarma) || 20), target_role: 'specific', target_user_ids: [uid],
        requires_review: true, deadline_at: deadline ? `${deadline}T23:59:59Z` : null, created_by: ctx.user.id,
      }).select().single()
      if (error) return res.status(500).json({ error: error.message })
      await a.from('task_assignments').insert({ task_id: task.id, user_id: uid, status: 'assigned', deadline_at: deadline ? `${deadline}T23:59:59Z` : null })
      referenceId = task.id
      await a.from('notifications').insert({ user_id: uid, message: `Новое задание: «${title}»`, link: `/task/${referenceId}` })
    } else if (actionType === 'test') {
      if (!testId) return res.status(400).json({ error: 'Выберите тест' })
      referenceId = testId
      await a.from('notifications').insert({ user_id: uid, message: `Назначен тест: «${title}»${deadline ? `, срок до ${deadline}` : ''}`, link: `/test/${testId}` })
    } else if (actionType === 'training') {
      await a.from('notifications').insert({ user_id: uid, message: `Назначено обучение: «${title}»${trainingNote ? ' — ' + trainingNote : ''}${deadline ? `, срок до ${deadline}` : ''}` })
    }

    const { error: devError } = await a.from('development_actions').insert({
      company_id: companyId, user_id: uid, action_type: actionType, title: title.trim(),
  ...
  res.status(200).json({ success: true, count: targets.length })
}
