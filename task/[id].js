...
export default function TaskDetail() {
  ...
  return (
    <div className="theme-light" style={{ minHeight: '100vh', position: 'relative', overflow: 'hidden' }}>
      <div style={{ position: 'fixed', top: '-10%', left: '-8%', width: 420, height: 420, borderRadius: '50%', background: 'radial-gradient(circle, rgba(234,88,12,0.10), transparent 70%)', filter: 'blur(10px)', pointerEvents: 'none' }} />
      ...
      <div className="premium-card" style={{ display: 'flex', flexDirection: 'column', minHeight: 320, position: 'relative', overflow: 'hidden', border: tierBorder, animation: tier === 'priority' ? 'taskDetailGlow 2.4s ease-in-out infinite' : 'none' }}>
        {tier === 'premium' && (
          <span style={{ position: 'absolute', top: 0, right: 0, fontSize: 10, fontWeight: 700, letterSpacing: 0.5, padding: '5px 14px 5px 16px', borderRadius: '0 0 0 14px', background: 'linear-gradient(135deg, #ea580c, #b45309)', color: '#fff', zIndex: 2 }}>ПРЕМИУМ</span>
        )}
        ...
        <div style={{ marginBottom: 12, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {t.is_auto_goal ? (
            ...
            <span style={{ fontSize: 11, padding: '3px 12px', borderRadius: 20, background: 'rgba(234,88,12,0.1)', color: 'var(--accent-gold)', border: '1px solid var(--border-gold)' }}>Партнёрское · {t.partner_name}</span>
          ) : (
          ...
          {t.requires_proof && (
            <div>
              ...
              <button onClick={() => fileRef.current?.click()}
                ...
                style={{ border: '1px dashed var(--border-gold)', background: 'rgba(234,88,12,0.06)', color: 'var(--text-secondary)' }}>
