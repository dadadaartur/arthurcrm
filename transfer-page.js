...
// Раньше получатель выбирался вводом email вручную — неудобно, если email
// не знаешь наизусть. Теперь — поиск по коллегам из своей же компании
// (имя/фамилия/email), с выпадающим списком. Список коллег читается
// напрямую с клиента: RLS-политика "Company members view colleagues" на
// profiles уже разрешает видеть всех сотрудников своей компании, отдельный
// API-роут для этого не нужен.
  ...
  const handleTransfer = async (e) => {
    ...
    try {
      ...
      const res = await fetch('/api/transfer', {
        ...
        body: JSON.stringify({ recipientUserId: selected.user_id, amount: transferAmount, comment })
      })
