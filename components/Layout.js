    ...
    <div className="min-h-screen flex flex-col" style={{ background: '#0a1628' }}>
      ...
      <header className="flex justify-between items-center px-6 py-2 relative z-10">
        <div className="flex items-center gap-4">
          ...
          <nav className="flex gap-1.5 text-2xs font-medium">
            <Link href="/path-to-perfection" className="action-btn !py-1 !px-2 !text-2xs">Путь к совершенству</Link>
            <Link href="/healthcare" className="action-btn !py-1 !px-2 !text-2xs">Забота о здоровье</Link>
            <Link href="/supd" className="action-btn !py-1 !px-2 !text-2xs">Кармическая CRM</Link>
          </nav>
        </div>
        <div className="flex items-center gap-2 text-2xs font-medium">
          {user && <span className="text-white font-semibold">Артур</span>}
          {user && (
            <button onClick={handleLogout} className="action-btn !py-1 !px-2 !text-2xs">
              Выйти
            </button>
          )}
        </div>
