    ...
    <div style={{ position: 'absolute', left: '2.5%', top: 250, zIndex: 20, width: 360, padding: '0 40px', boxSizing: 'border-box', animation: 'driftGoals 60s ease-in-out infinite alternate' }}>
      ...
      {cur && (
        <div style={{ textAlign: 'center', marginBottom: 12 }}>
          ...
          {next && <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)' }}> → {next.name}: ещё {next.energy_threshold - energy} эн.</span>}
          <button onClick={() => setPathOpen(true)} style={{ display: 'block', margin: '4px auto 0', background: 'none', border: 'none', color: '#a0e9ff', fontSize: 10, cursor: 'pointer', textDecoration: 'underline' }}>путь прогресса</button>
        </div>
      )}
        ...
        {data.metrics.map(m => (
          <div key={m.id}>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, marginBottom: 4 }}>
              ...
              <span style={{ fontSize: 11, color: BAND_COLORS[m.band], textShadow: `0 0 6px ${BAND_COLORS[m.band]}`, fontWeight: 700, whiteSpace: 'nowrap', flexShrink: 0 }}>{m.current != null ? `${m.current}${m.unit}` : '—'} · {BAND_LABELS[m.band]}</span>
            </div>
