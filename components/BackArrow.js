  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 22, flexWrap: 'wrap' }}>
      <Link href={href} className="back-arrow-circle"
        style={{ width: 30, height: 30, borderRadius: '50%', border: '1px solid rgba(255,215,0,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all .25s', flexShrink: 0, background: 'rgba(255,255,255,0.03)' }}
      ...
      <h1 className="back-arrow-title" style={{ fontSize: 20, fontWeight: 600, margin: 0, background: 'linear-gradient(135deg, #a0e9ff, #ffb3c6)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>{title}</h1>
      {extra}
    </div>
  )
}
