  ...
  return (
    <span style={{ position: 'relative', display: 'inline-flex' }}>
      <button type="button" onClick={() => setOpen(o => !o)} onBlur={() => setTimeout(() => setOpen(false), 150)}
        style={{ width: 17, height: 17, borderRadius: '50%', border: '1px solid var(--border-gold, rgba(176,128,16,.45))', background: 'rgba(234,88,12,0.12)', color: '#ea580c', fontSize: 10, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 0, lineHeight: 1 }}>
        ?
      </button>
      {open && (
