...
export default function ProgressBar3D({ value = 0, marks = [], height = 14, unit = '' }) {
  ...
  // Масштаб ВСЕГДА на 25% больше И максимального порога, И реального значения.
  // 48 звонков при ультра 40 → шкала до 60, указатель на 80% — ничего не обрезается.
  const maxVal = (Math.max(maxMark, v) * 1.25) || 1
  ...
  return (
    <div style={{ position: 'relative', height, borderRadius: height, background: 'rgba(0,0,0,0.55)', border: '1px solid rgba(255,255,255,0.07)', boxShadow: 'inset 0 2px 6px rgba(0,0,0,0.8), 0 1px 0 rgba(255,255,255,0.06)', overflow: 'hidden' }}>
      {/* заблюрованный потенциал — «всегда можно больше» */}
      ...
      {/* яркий реальный прогресс — строго value/maxVal */}
      ...
      {/* указатель «где я» — на той же математике */}
