            ...
            <div style={{ flex: 1, minHeight: 0, overflow: 'auto' }}>
              <div style={{ width: 'fit-content', margin: '0 auto' }}>
                <div style={{ display: 'grid', gridTemplateColumns: gridTemplate, gap: 6, marginBottom: 6, position: 'sticky', top: 0, background: 'rgba(10,14,28,0.98)', padding: '6px 0', zIndex: 2 }}>
                  ...
                  {cols.map(c => <div key={c.key} style={{ textAlign: 'center', padding: '4px', borderRadius: 8, background: c.inverse ? 'rgba(160,233,255,0.06)' : 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}><div style={{ fontSize: 10, color: '#a0e9ff', fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.label}</div><div style={{ fontSize: 9, color: '#666' }}>{c.inverse ? 'меньше=лучше' : (c.unit || '')}</div></div>)}
                ...
                {employees.map(emp => (
                  <div key={emp.user_id} style={{ display: 'grid', gridTemplateColumns: gridTemplate, gap: 6, marginBottom: 6, alignItems: 'center' }}>
                    <div style={{ fontSize: 12, color: '#fff', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', width: 170 }}>{empName(emp)}</div>
