        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap', margin: '16px 0', padding: '14px 16px', borderRadius: 14, background: 'var(--bg-page)' }}>
          <div>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              <button onClick={() => setTab('table')} style={{ padding: '7px 15px', borderRadius: 20, fontSize: 12, cursor: 'pointer', fontWeight: tab === 'table' ? 600 : 400, background: tab === 'table' ? 'rgba(234,88,12,0.1)' : 'var(--bg-card)', border: `1px solid ${tab === 'table' ? 'var(--border-gold)' : 'var(--border-subtle)'}`, color: tab === 'table' ? '#ea580c' : 'var(--text-secondary)' }}>Таблица</button>
              <button onClick={() => setTab('import')} style={{ padding: '7px 15px', borderRadius: 20, fontSize: 12, cursor: 'pointer', fontWeight: tab === 'import' ? 600 : 400, background: tab === 'import' ? 'rgba(234,88,12,0.1)' : 'var(--bg-card)', border: `1px solid ${tab === 'import' ? 'var(--border-gold)' : 'var(--border-subtle)'}`, color: tab === 'import' ? '#ea580c' : 'var(--text-secondary)' }}>Импорт</button>
            </div>
          </div>
        ...
        {loading ? (
          ...
          <>
            <div style={{ display: 'flex', gap: 14, marginBottom: 10, fontSize: 11, color: 'var(--text-secondary)' }}>
              ...
              <span><span style={{ display: 'inline-block', width: 9, height: 9, borderRadius: 3, background: 'rgba(234,88,12,0.15)', border: '1px solid var(--border-gold)', marginRight: 5, verticalAlign: -1 }} />считает сама система — это формула из соседних колонок, вводить не нужно</span>
                ...
                <thead>
                  <tr style={{ position: 'sticky', top: 0, background: 'var(--bg-page)', zIndex: 2 }}>
                    ...
                    {formulaMetrics.map(m => (
                      <th key={m.id} style={{ padding: '10px 8px', minWidth: 110, background: 'rgba(234,88,12,0.06)' }}>
                        <div style={{ fontSize: 11, fontWeight: 600, color: '#ea580c', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={m.name}>{m.name}</div>
                        <div style={{ fontSize: 9, color: '#ea580c', fontWeight: 400 }}>считает система</div>
                      </th>
                    ))}
                  </tr>
                </thead>
                  ...
                  {employees.map((emp, i) => (
                    <tr key={emp.user_id} style={{ background: i % 2 ? 'var(--bg-page)' : 'transparent' }}>
                      ...
                      {cols.map(c => {
                        ...
                        return (
                          <td key={c.key} style={{ padding: '4px 6px', position: 'relative' }}>
                            ...
                            />
                            {st === 'saving' && (
                              <span style={{ position: 'absolute', right: 10, top: '50%', marginTop: -6, width: 12, height: 12, borderRadius: '50%', border: '2px solid rgba(138,98,8,0.25)', borderTopColor: '#ea580c', animation: 'frmSpin 0.7s linear infinite' }} />
                            )}
                            {st === 'saved' && (
                      ...
                      {formulaMetrics.map(m => { const v = computeValue(m, emp.user_id); return (
                        <td key={m.id} style={{ padding: '4px 6px', textAlign: 'center', fontWeight: 600, color: v != null ? '#ea580c' : 'var(--text-muted)' }}>{v != null ? v + (m.unit || '') : '—'}</td>
                      )})}
                    </tr>
                  ))}
                </tbody>
          ...
          <div style={{ flex: 1, minHeight: 0, overflow: 'auto' }}>
            ...
            <div style={{ display: 'flex', gap: 6, marginBottom: 14 }}>
              <button onClick={() => setImportTab('paste')} style={{ padding: '7px 15px', borderRadius: 20, fontSize: 12, cursor: 'pointer', fontWeight: importTab === 'paste' ? 600 : 400, background: importTab === 'paste' ? 'rgba(234,88,12,0.1)' : 'var(--bg-card)', border: `1px solid ${importTab === 'paste' ? 'var(--border-gold)' : 'var(--border-subtle)'}`, color: importTab === 'paste' ? '#ea580c' : 'var(--text-secondary)' }}>Вставить данные</button>
              <button onClick={() => setImportTab('google')} style={{ padding: '7px 15px', borderRadius: 20, fontSize: 12, cursor: 'pointer', fontWeight: importTab === 'google' ? 600 : 400, background: importTab === 'google' ? 'rgba(234,88,12,0.1)' : 'var(--bg-card)', border: `1px solid ${importTab === 'google' ? 'var(--border-gold)' : 'var(--border-subtle)'}`, color: importTab === 'google' ? '#ea580c' : 'var(--text-secondary)' }}>Ссылка на Google Таблицу</button>
            </div>
            {importTab === 'paste' ? (
              <>
