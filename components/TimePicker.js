...
export default function TimePicker({ value, onChange, placeholder = 'Время', disabled = false }) {
  ...
  const chip = (active, label, onClick) => (
    <button
      ...
      style={{
        background: active ? 'linear-gradient(135deg, #ea580c, #7c3aed)' : 'var(--bg-page, #f3f4f8)',
    ...
    <>
      ...
      >
        ...
        <svg width="15" height="15" viewBox="0 0 16 16" fill="none" className="flex-shrink-0">
          <circle cx="8" cy="8" r="6.5" stroke="#ea580c" strokeWidth="1.2" />
      ...
      {open && typeof document !== 'undefined' && createPortal(
        ...
        >
          ...
          <div className="flex justify-between items-center">
            ...
            <button type="button" onClick={now} className="text-[11px] transition-colors" style={{ color: '#ea580c' }}>Сейчас</button>
            <button
              type="button"
              onClick={apply}
