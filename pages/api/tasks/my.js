...
export default async function handler(req, res) {
  ...
  const [{ data: active }, { data: history }] = await Promise.all([
    a.from('task_assignments')
      .select('id, status, started_at, deadline_at, task_id, progress_count, tasks(id, title, description, reward_karma, image_url, video_url, is_auto_goal, auto_goal_condition, auto_metric_id, auto_target_rank, reward_type, reward_config, bonus_rewards, partner_name, requires_proof, proof_type, visual_tier, target_count, target_count_label)')
