...
export default async function handler(req, res) {
  ...
  if (action === 'approve') {
    ...
    // Энергия пропорциональна награде задания, не плоская единица (по
    // анализу от 6 сентября 2026 — осознанно пересматриваю собственную
    // же более раннюю просьбу от 27 августа 2026 о фиксированном
    // значении: тогда цель была «пусть задания вообще дают энергию»,
    // сегодня цель точнее — «сложное задание должно весить больше
    // шуточного». Награда, которую уже назначил админ, — естественный,
    // уже существующий показатель ценности задания, не новый).
    const grantedEnergy = await creditEnergy(a, asg.user_id, Math.max(1, Math.round(k / 10)), 'task', asg.tasks?.title)
