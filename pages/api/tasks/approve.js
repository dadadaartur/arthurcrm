...
export default async function handler(req, res) {
  ...
  // Если уже обработано – просто возвращаем успех, ничего не делаем
  if (assignment.status !== 'pending_review') {
    return res.status(200).json({ result: 'OK', alreadyProcessed: true })
  }

  const newStatus = action === 'approve' ? 'completed' : 'in_progress'

  // Атомарно обновляем статус ТОЛЬКО если он ещё pending_review
