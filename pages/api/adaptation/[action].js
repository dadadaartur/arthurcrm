...
async function logHistory(a, body) { await a.from('adaptation_history').insert(body) }
...
async function progress(a, planId) {
  const { data: ev } = await a.from('adaptation_plan_events').select('id, status, is_mandatory, rating').eq('plan_id', planId)
  const list = ev || []
...
export default async function handler(req, res) {
  ...
  // ===== СПИСОК ПЛАНОВ (РОП) / СВОЙ ПЛАН (сотрудник) =====
  if (action === 'plans' && req.method === 'GET') {
    ...
    for (const p of data || []) {
      ...
      out.push({ ...p, employee_name: emp ? ([emp.first_name, emp.last_name].filter(Boolean).join(' ') || emp.display_name || emp.email) : '—', progress: pr })
    }
    return res.status(200).json(out)
  }

  // ===== СОЗДАНИЕ ПЛАНА (черновик + материализация событий) =====
  ...
  // ===== ПРАВКА ЧЕРНОВИКА (update/add/delete событий) =====
  ...
  if (action === 'activate' && req.method === 'POST') {
    ...
    await logHistory(a, { plan_id: planId, employee_id: plan.employee_id, manager_id: ctx.user.id, type: 'plan_meeting', description: 'План адаптации создан и активирован', author_role: 'system' })
  ...
  if (action === 'my' && req.method === 'GET') {
    ...
    return res.status(200).json({ plan, events: enriched, progress: pr, currentEvent: current })
  }
  ...
  if (action === 'complete' && req.method === 'POST') {
    ...
    await logHistory(a, { plan_id: ev.plan_id, event_id: eventId, employee_id: ctx.user.id, type: 'control', description: `Событие «${ev.title}» выполнено.${feedback ? ' Фидбек: ' + feedback : ''}`, author_role: 'employee' })
  ...
  if (action === 'approve' && req.method === 'POST') {
    ...
    await logHistory(a, { plan_id: ev.plan_id, event_id: eventId, manager_id: ctx.user.id, type: 'feedback', description: `Обратная связь по «${ev.title}».${feedback ? ' ' + feedback : ''}`, author_role: 'manager', rating: rating || null })
    await notify(a, ev.adaptation_plans.employee_id, `РОП оставил обратную связь по «${ev.title}» (оценка ${rating || '—'}).`, '/onboarding')
    return res.status(200).json({ success: true })
  }

  // ===== КОРРЕКТИРОВКА (новое событие + запись в историю) =====
  if (action === 'correction' && req.method === 'POST') {
    ...
    if (eventId) await a.from('adaptation_history').update({ corrected_by: hist.id }).eq('id', eventId).eq('plan_id', planId)
