...
export default async function handler(req, res) {
  ...
  // ВСЕ активные участники компании (включая админов) — раньше админы
  // исключались и раздел выглядел пустым, когда данные заполнял сам админ.
  const { data: employees } = await a.from('profiles')
    .select('user_id, email, display_name, first_name, last_name')
    .eq('company_id', companyId).is('deleted_at', null)
  const { data: metrics } = await a.from('kpi_metrics')
    .select('*').eq('company_id', companyId).eq('is_active', true)

  const userIds = (employees || []).map(e => e.user_id)
  let balMap = {}
  if (userIds.length) {
    const { data: bals } = await a.from('karma_balance').select('user_id, balance').in('user_id', userIds)
    balMap = Object.fromEntries((bals || []).map(b => [b.user_id, Number(b.balance) || 0]))
  }

  const rows = []
  for (const emp of employees || []) {
    let q = a.from('kpi_entries').select('*').eq('user_id', emp.user_id)
    ...
    const { data: tests } = await a.from('kpi_test_results').select('*').eq('user_id', emp.user_id).eq('passed', true)
    const latest = {}
    ...
    rows.push({
      ...
      entries: entries || [],
      latest: Object.values(latest).map(e => ({
        ...e,
        band: (() => { const m = (metrics || []).find(x => x.id === e.metric_id); return m ? bandFor(e.value, m) : 'none' })()
      }))
    })
  }
