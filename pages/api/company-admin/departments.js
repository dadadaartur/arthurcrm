...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    const { data: allEmployees } = await a.from('profiles').select('user_id, email, first_name, last_name, display_name, department_id, manager_id, is_company_admin, avatar_url, role_id')
      .eq('company_id', companyId).is('deleted_at', null)
    // Название роли — раньше не подтягивалось вовсе, из-за чего при
    // раскидывании сотрудников по отделам админ не видел, кто кем
    // назначен, и был вынужден отдельно проверять на странице
    // сотрудников (пункт 4 фидбека от 2 сентября 2026).
    const { data: allRoles } = await a.from('roles').select('id, name').eq('company_id', companyId)
    const roleNameById = {}
    ;(allRoles || []).forEach(r => { roleNameById[r.id] = r.name })
    const withRoleName = (allEmployees || []).map(e => ({ ...e, role_name: e.is_company_admin ? 'Администратор' : (roleNameById[e.role_id] || null) }))
    if (scope === null) {
      return res.status(200).json({ departments: allDepartments || [], employees: withRoleName, scope: null })
    }
    ...
    const visibleEmployees = withRoleName.filter(e => scope.includes(e.department_id) || e.user_id === ctx.user.id)
