...
export default async function handler(req, res) {
  ...
  // Индекс записей по показателю → сотруднику, построенный один раз —
  // вместо повторной фильтрации всего массива entries на каждой
  // итерации вложенных циклов ниже (при большом объёме сотрудников ×
  // показателей × дней это были десятки тысяч лишних операций,
  // вероятная настоящая причина медленной работы и таймаута).
  const entriesByMetricUser = {}
  ;(entries || []).forEach(e => {
    const key = `${e.metric_id}:${e.user_id}`
    ;(entriesByMetricUser[key] = entriesByMetricUser[key] || []).push(e)
  })
  const entriesForMetricUser = (metricId, userId) => entriesByMetricUser[`${metricId}:${userId}`] || []
  const monthEntriesByMetricUser = {}
  ;(monthEntries || []).forEach(e => {
    const key = `${e.metric_id}:${e.user_id}`
    ;(monthEntriesByMetricUser[key] = monthEntriesByMetricUser[key] || []).push(e)
  })
  const monthEntriesForMetricUser = (metricId, userId) => monthEntriesByMetricUser[`${metricId}:${userId}`] || []
    ...
    for (const m of metrics || []) {
      ...
      if (!onTrack) {
        const ranked = pool.map(e => {
          const val = agg(m, monthEntriesForMetricUser(m.id, e.user_id))
  ...
  for (const m of metrics || []) {
    ...
    // и для тренда, и для сравнения с командой в последнем отрезке.
    // Берётся из индекса (построен один раз выше), не повторной
    // фильтрацией всего массива на каждого сотрудника.
    const byEmpByPeriod = {}
    pool.forEach(e => { const ue = entriesForMetricUser(m.id, e.user_id); byEmpByPeriod[e.user_id] = periods.map(p => agg(m, ue.filter(x => x.entry_date >= p.from && x.entry_date <= p.to))) })
    ...
    const ranked = pool.map(e => {
      const val = agg(m, entriesForMetricUser(m.id, e.user_id))
  ...
  for (const e of pool) {
    ...
    for (const m of metrics || []) {
      const ue = entriesForMetricUser(m.id, e.user_id)
      if (!ue.length) continue
      const periodVals = periods.map(p => agg(m, ue.filter(x => x.entry_date >= p.from && x.entry_date <= p.to)))
