...
export default async function handler(req, res) {
  ...
  const { data: employees } = await sb.from('profiles').select('user_id, email, display_name, first_name, last_name').eq('company_id', ctx.profile.company_id).is('deleted_at', null).eq('is_company_admin', false)
