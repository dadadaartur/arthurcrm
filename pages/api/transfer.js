...
export default async function handler(req, res) {
  ...
  // Получатель: по id (TransferModal) или по email (страница /transfer)
  let recipient = null
  if (recipientId) {
    const { data, error } = await supabaseAdmin
      ...
      .is('deleted_at', null)
      .maybeSingle()
    if (error) return res.status(500).json({ error: 'Ошибка поиска получателя: ' + error.message })
    recipient = data
  } else {
  ...
  const { data: senderBal } = await supabaseAdmin
    .from('karma_balance').select('balance').eq('user_id', ctx.user.id).single()
  ...
  const { data: recipBal } = await supabaseAdmin
    .from('karma_balance').select('balance').eq('user_id', recipient.user_id).single()
  const { error: creditError } = await supabaseAdmin
    .from('karma_balance')
    .update({ balance: (recipBal?.balance ?? 0) + transferAmount, updated_at: new Date().toISOString() })
  ...
  await supabaseAdmin.from('notifications').insert([
    {
      user_id: ctx.user.id,
      message: `Вы перевели ${transferAmount} кармиков → ${recipient.email}${comment ? '. ' + comment : ''}`,
      link: '/history'
    },
    {
