  ...
  return (
    <div className="modal-overlay" onClick={onCancel}>
      <div className="modal-content" onClick={e => e.stopPropagation()}>
        ...
        <p className="mb-6 text-white">Вы уверены, что хотите удалить?</p>
...
export default function CompanyAdmin() {
  ...
  return (
    <div className="container mx-auto px-4 py-12">
        ...
        <div className="dash-card">
          ...
          {employees.length === 0 ? (
            ...
            <div className="flex flex-col gap-2">
              {employees.map(emp => (
                <div key={emp.id} className="flex justify-between items-center p-3 rounded-lg bg-gray-800">
                  <div>
                    <span className="text-white font-medium">{emp.display_name || emp.email}</span>
                    <span className="ml-3 text-xs text-gray-400">
                      {emp.roles?.name || (emp.role_id === 1 ? 'Суперадмин' : emp.role_id === 2 ? 'Администратор' : 'Сотрудник')}
                    </span>
                    {emp.companies?.name && (
                      <span className="ml-3 text-xs text-gray-500">({emp.companies.name})</span>
                    )}
                  </div>
      ...
      {deletePositionId && (
        <ConfirmModal
          onConfirm={() => handleDeletePosition(deletePositionId)}
