...
// Кармический Центробанк — только для основателя платформы.
export default function CentralBank() {
  ...
  const fetchAll = async () => {
    ...
    if (res.ok) {
      ...
      setBank(d.bank); setCompanies(d.companies); setTariffs(d.tariffs); setLedger(d.ledger)
    } else {
  ...
  const handleEmit = async (companyId) => {
    ...
    try {
      ...
      if (res.ok) {
        setMessage(`Эмиссия выполнена: +${result.emitted} кармиков компании. Остаток ЦБ: ${result.new_bank_balance}`)
        await fetchAll()
      } else {
      ...
      <div style={{ maxWidth: 1600, margin: '0 auto' }}>
        ...
        {!bank && (
          <div className="premium-card mb-6 text-sm text-gray-400">
            Центробанк не найден в базе — прогони SQL-скрипт из инструкции, затем обнови страницу.
          </div>
        )}
                  ...
                  return (
                    <tr key={c.id} className="border-b border-gray-800 hover:bg-white/[0.02]">
                      ...
                      <td className="py-3 pr-4 text-gray-400">
                        {c.valid_until ? new Date(c.valid_until).toLocaleDateString('ru') : '—'}
                      </td>
