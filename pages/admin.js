...
export default function Admin() {
  ...
  async function handleCreateCompany(e) {
    ...
    // 2. Создаём роль "Администратор" для этой компании
    const { data: role } = await supabase.from('roles').insert({ name: 'Администратор', company_id: company.id }).select().single()
    if (!role) return

    // 3. Генерируем токен и временный пароль
