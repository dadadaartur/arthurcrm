    ...
    <div className="theme-light" style={{ minHeight: '100vh', fontFamily: 'Inter, sans-serif', padding: '40px 32px' }}>
      <div style={{ maxWidth: 900, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
          <button onClick={() => setFilter('pending')} style={{ fontSize: 12, padding: '6px 14px', borderRadius: 20, border: `1px solid ${filter === 'pending' ? 'var(--border-gold)' : 'var(--border-subtle)'}`, background: filter === 'pending' ? 'rgba(234,88,12,0.08)' : 'var(--bg-card)', color: filter === 'pending' ? '#ea580c' : 'var(--text-secondary)', cursor: 'pointer' }}>Не выдано</button>
          <button onClick={() => setFilter('all')} style={{ fontSize: 12, padding: '6px 14px', borderRadius: 20, border: `1px solid ${filter === 'all' ? 'var(--border-gold)' : 'var(--border-subtle)'}`, background: filter === 'all' ? 'rgba(234,88,12,0.08)' : 'var(--bg-card)', color: filter === 'all' ? '#ea580c' : 'var(--text-secondary)', cursor: 'pointer' }}>Все</button>
        </div>
        ...
        <div style={{ background: 'var(--bg-card)', boxShadow: 'var(--shadow-card)', borderRadius: 16, border: '1px solid var(--border-subtle)', overflow: 'hidden' }}>
          ...
          {shown.map(a => (
            <div key={a.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16, padding: '16px 22px', borderTop: '1px solid var(--border-subtle)' }}>
              ...
              <button onClick={() => toggleFulfilled(a)} style={{ flexShrink: 0, fontSize: 12, padding: '7px 16px', borderRadius: 10, border: `1px solid ${a.fulfilled ? 'rgba(19,122,57,0.3)' : 'var(--border-gold)'}`, background: a.fulfilled ? 'rgba(19,122,57,0.08)' : 'rgba(234,88,12,0.08)', color: a.fulfilled ? '#137a39' : '#ea580c', cursor: 'pointer', whiteSpace: 'nowrap' }}>
