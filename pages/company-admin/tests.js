...
function TestsAdmin() {
  ...
  const [form, setForm] = useState({ title: '', description: '', video_url: '', training_id: '', time_limit: 10, passing_score: 70, attempts_allowed: 3, is_random: true, show_correct_answers: true, energy_reward: 0, karma_reward: 0 })
  ...
  useEffect(() => {
    load()
    if (router.query.training) setForm(f => ({ ...f, training_id: router.query.training }))
  }, [])
  ...
  const openEdit = async (id) => {
    ...
    setEditId(id)
    setForm({ title: t.title, description: t.description || '', video_url: t.video_url || '', training_id: t.training_id || '', time_limit: t.time_limit, passing_score: t.passing_score, attempts_allowed: t.attempts_allowed, is_random: t.is_random, show_correct_answers: t.show_correct_answers, energy_reward: t.energy_reward, karma_reward: t.karma_reward })
  ...
  const openAnalytics = async (id) => {
    setAnalyticsId(id); setView('analytics'); setAnalytics(null)
    const h = await auth()
  ...
  const save = async () => {
    ...
    if (r.ok) { showSuccess(editId ? 'Тест обновлён' : 'Тест опубликован, сотрудники уведомлены'); setView('list'); setEditId(null); setQuestions([emptyQ()]); load() }
      ...
      <div style={{ maxWidth: 1600, margin: '0 auto' }}>
        <BackArrow href="/company-admin/mastery" title="Тесты и срезы знаний" extra={
          <div style={{ marginLeft: 'auto', display: 'flex', gap: 8 }}>
            ...
            <Seg active={view === 'builder'} onClick={() => { setView('builder'); setEditId(null); setQuestions([emptyQ()]) }}>Конструктор</Seg>
          </div>
        } />
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(360px, 1fr))', gap: 14 }}>
            ...
            {tests.map(t => (
              <div key={t.id} style={{ background: 'rgba(15,20,35,0.85)', borderRadius: 16, padding: 20, border: '1px solid rgba(255,255,255,0.08)', transition: 'border-color 0.25s', display: 'flex', flexDirection: 'column', gap: 12 }}
                ...
                <div style={{ fontSize: 11, color: '#a0e9ff' }}>Порог {t.passing_score}% · Награда: {t.karma_reward} карм. / {t.energy_reward} эн.</div>
                <div style={{ display: 'flex', gap: 8, marginTop: 'auto', flexWrap: 'wrap' }}>
                  <button onClick={() => openAnalytics(t.id)} style={{ ...ghostBtn, padding: '6px 14px', fontSize: 11 }} onMouseEnter={hoverOn} onMouseLeave={hoverOff}>Аналитика</button>
                  <button onClick={() => openEdit(t.id)} style={{ ...ghostBtn, padding: '6px 14px', fontSize: 11, borderColor: 'rgba(160,233,255,0.4)', color: '#a0e9ff' }} onMouseEnter={hoverOn} onMouseLeave={hoverOff}>Редактировать</button>
                  <button onClick={() => del(t.id)} style={{ ...ghostBtn, padding: '6px 14px', fontSize: 11, borderColor: 'rgba(244,67,54,0.3)', color: '#f87171' }} onMouseEnter={hoverOn} onMouseLeave={hoverOff}>Удалить</button>
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            <div style={{ background: 'rgba(15,20,35,0.85)', borderRadius: 20, padding: 28, border: '1px solid rgba(255,255,255,0.08)' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14 }}>
                <div style={{ gridColumn: 'span 2' }}><label style={{ fontSize: 12, color: '#888', display: 'block', marginBottom: 6 }}>Название</label><input className="input-field" style={{ width: '100%' }} value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} /></div>
                <div style={{ gridColumn: 'span 2' }}><label style={{ fontSize: 12, color: '#888', display: 'block', marginBottom: 6 }}>Видео / материал (ссылка)</label><input className="input-field" style={{ width: '100%' }} value={form.video_url} onChange={e => setForm({ ...form, video_url: e.target.value })} /></div>
                <div><label style={{ fontSize: 11, color: '#888', display: 'block', marginBottom: 6 }}>Время (мин)</label><input type="number" className="input-field" style={{ width: '100%' }} value={form.time_limit} onChange={e => setForm({ ...form, time_limit: +e.target.value })} /></div>
                <div><label style={{ fontSize: 11, color: '#888', display: 'block', marginBottom: 6 }}>Порог %</label><input type="number" className="input-field" style={{ width: '100%' }} value={form.passing_score} onChange={e => setForm({ ...form, passing_score: +e.target.value })} /></div>
                <div><label style={{ fontSize: 11, color: '#888', display: 'block', marginBottom: 6 }}>Попытки</label><input type="number" className="input-field" style={{ width: '100%' }} value={form.attempts_allowed} onChange={e => setForm({ ...form, attempts_allowed: +e.target.value })} /></div>
                <div><label style={{ fontSize: 11, color: '#888', display: 'block', marginBottom: 6 }}>Кармики</label><input type="number" className="input-field" style={{ width: '100%' }} value={form.karma_reward} onChange={e => setForm({ ...form, karma_reward: +e.target.value })} /></div>
                <div><label style={{ fontSize: 11, color: '#888', display: 'block', marginBottom: 6 }}>Энергия</label><input type="number" className="input-field" style={{ width: '100%' }} value={form.energy_reward} onChange={e => setForm({ ...form, energy_reward: +e.target.value })} /></div>
                ...
                <div style={{ gridColumn: 'span 4' }}><label style={{ fontSize: 12, color: '#888', display: 'block', marginBottom: 6 }}>Описание</label><textarea className="input-field" style={{ width: '100%' }} rows={2} value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} /></div>
              </div>
            </div>
              ...
              <div key={i} style={{ background: 'rgba(15,20,35,0.85)', borderRadius: 16, padding: 20, border: '1px solid rgba(255,255,255,0.08)' }}>
                <div style={{ display: 'flex', gap: 10, marginBottom: 12, alignItems: 'center', flexWrap: 'wrap' }}>
                  ...
                  <select className="input-field" value={q.type} onChange={e => setQ(i, { type: e.target.value })}>
                    <option value="single">Одиночный выбор</option><option value="multi">Множественный</option><option value="fill">Пропуск</option><option value="open">Открытый</option>
                  </select>
                  <input type="number" className="input-field" style={{ width: 90 }} value={q.points} onChange={e => setQ(i, { points: +e.target.value })} title="Баллы" />
