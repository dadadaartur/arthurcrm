...
const Seg = ({ active, onClick, children, color = '#8a6208' }) => (
  <button onClick={onClick} style={{
    ...
    border: `1px solid ${active ? color + '88' : 'var(--border-subtle)'}`, color: active ? color : 'var(--text-secondary)',
      ...
      <div style={{ maxWidth: 1600, margin: '0 auto' }}>
        ...
        {view === 'list' && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(360px, 1fr))', gap: 14 }}>
            {tests.length === 0 && <div style={{ gridColumn: '1 / -1', background: 'var(--bg-card)', boxShadow: 'var(--shadow-card)', borderRadius: 20, padding: 60, textAlign: 'center', color: 'var(--text-muted)' }}>Тестов пока нет — создайте первый в конструкторе</div>}
            {tests.map(t => (
              <div key={t.id} style={{ background: 'var(--bg-card)', boxShadow: 'var(--shadow-card)', borderRadius: 16, padding: 20, border: '1px solid var(--border-subtle)', transition: 'border-color 0.25s', display: 'flex', flexDirection: 'column', gap: 12 }}
                onMouseEnter={e => e.currentTarget.style.borderColor = 'rgba(124,58,237,0.35)'} onMouseLeave={e => e.currentTarget.style.borderColor = 'var(--border-subtle)'}>
                <div style={{ color: 'var(--text-primary)', fontWeight: 600, fontSize: 15 }}>{t.title}</div>
                <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>Вопросов: {t.question_count} · Попыток: {t.attempts_count} · Сдача: {t.pass_rate}%</div>
                <div style={{ fontSize: 11, color: '#0e7490' }}>Порог {t.passing_score}% · Награда: {t.karma_reward} карм. / 5 эн.</div>
                <div style={{ display: 'flex', gap: 8, marginTop: 'auto', flexWrap: 'wrap' }}>
                  <button onClick={() => openAnalytics(t.id)} style={ghostBtn} onMouseEnter={hoverOn} onMouseLeave={hoverOff}>Аналитика</button>
                  <button onClick={() => openEdit(t.id)} style={{ ...ghostBtn, borderColor: 'rgba(14,116,144,0.35)', color: '#0e7490' }} onMouseEnter={hoverOn} onMouseLeave={hoverOff}>Редактировать</button>
                  <button onClick={() => del(t.id)} style={{ ...ghostBtn, borderColor: 'rgba(220,38,38,0.3)', color: '#dc2626' }} onMouseEnter={hoverOn} onMouseLeave={hoverOff}>Удалить</button>
                </div>
              </div>
            ))}
          </div>
        ...
        {view === 'builder' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            <div style={{ background: 'var(--bg-card)', boxShadow: 'var(--shadow-card)', borderRadius: 20, padding: 28, border: '1px solid var(--border-subtle)' }}>
              <h3 style={{ fontSize: 17, fontWeight: 600, marginBottom: 16, color: 'var(--text-primary)' }}>{editId ? 'Редактировать тест' : 'Новый тест'}</h3>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14 }}>
                ...
                <div style={{ gridColumn: 'span 2', display: 'flex', gap: 8, alignItems: 'flex-end' }}>
                  <div style={{ flex: 1 }}>
                    <label style={{ fontSize: 12, color: 'var(--text-secondary)', display: 'block', marginBottom: 6 }}>Видео (ссылка при загрузке)</label>
                ...
                <div><label style={{ fontSize: 11, color: 'var(--text-secondary)', display: 'block', marginBottom: 6 }}>Время (мин)</label><input type="number" className="input-field" style={{ width: 90 }} value={form.time_limit} onChange={e => setForm({ ...form, time_limit: +e.target.value })} /></div>
                <div><label style={{ fontSize: 11, color: 'var(--text-secondary)', display: 'block', marginBottom: 6 }}>Порог %</label><input type="number" className="input-field" style={{ width: 90 }} value={form.passing_score} onChange={e => setForm({ ...form, passing_score: +e.target.value })} /></div>
                <div><label style={{ fontSize: 11, color: 'var(--text-secondary)', display: 'block', marginBottom: 6 }}>Попытки</label><input type="number" className="input-field" style={{ width: 90 }} value={form.attempts_allowed} onChange={e => setForm({ ...form, attempts_allowed: +e.target.value })} /></div>
                <div>
                  <label style={{ fontSize: 11, color: 'var(--text-secondary)', display: 'block', marginBottom: 6 }}>Кармики (макс 5)</label>
                  ...
                  <div style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 4 }}>Баланс компании: <b style={{ color: 'var(--accent-gold)' }}>{companyKarma}</b>. Энергия фиксирована: 5.</div>
                </div>
                <div style={{ gridColumn: 'span 4', display: 'flex', gap: 16, alignItems: 'center', flexWrap: 'wrap' }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: 'var(--text-primary)', cursor: 'pointer' }}><input type="checkbox" checked={form.is_random} onChange={e => setForm({ ...form, is_random: e.target.checked })} style={{ accentColor: '#7c3aed' }} />Случайный порядок</label>
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: 'var(--text-primary)', cursor: 'pointer' }}><input type="checkbox" checked={form.show_correct_answers} onChange={e => setForm({ ...form, show_correct_answers: e.target.checked })} style={{ accentColor: '#7c3aed' }} />Показывать ответы после</label>
                </div>
            ...
            {questions.map((q, i) => (
              <div key={i} style={{ background: 'var(--bg-card)', boxShadow: 'var(--shadow-card)', borderRadius: 16, padding: 20, border: '1px solid var(--border-subtle)' }}>
                <div style={{ display: 'flex', gap: 10, marginBottom: 12, alignItems: 'center', flexWrap: 'wrap' }}>
                  <span style={{ fontSize: 12, color: '#7c3aed', fontWeight: 700 }}>Вопрос {i + 1}</span>
                  ...
                  <button onClick={() => setQuestions(qs => qs.filter((_, idx) => idx !== i))} style={{ marginLeft: 'auto', background: 'none', border: 'none', color: '#dc2626', fontSize: 11, cursor: 'pointer' }}>Удалить</button>
                </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {q.options.map((o, oi) => (
                      <div key={oi} style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                        ...
                        <button onClick={() => setQuestions(qs => qs.map((qq, idx) => idx === i ? { ...qq, options: qq.options.filter((_, j) => j !== oi) } : qq))} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer' }}>×</button>
                      </div>
                    ))}
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={save} style={{ ...ghostBtn, borderColor: 'var(--border-gold)', color: 'var(--accent-gold)' }} onMouseEnter={hoverOn} onMouseLeave={hoverOff}>{editId ? 'Сохранить' : 'Опубликовать'}</button>
            </div>
          </div>
        )}
            ...
            {!analytics ? <div style={{ display: 'flex', justifyContent: 'center', padding: '40px 0' }}><Spinner /></div> : (
              <>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14 }}>
                  <div style={{ background: 'var(--bg-card)', boxShadow: 'var(--shadow-card)', borderRadius: 16, padding: 20, border: '1px solid var(--border-gold)' }}><div style={{ fontSize: 11, color: 'var(--text-secondary)' }}>Средний балл</div><div style={{ fontSize: 30, fontWeight: 700, color: 'var(--accent-gold)' }}>{analytics.avg}%</div></div>
                  <div style={{ background: 'var(--bg-card)', boxShadow: 'var(--shadow-card)', borderRadius: 16, padding: 20, border: '1px solid rgba(14,116,144,0.25)' }}><div style={{ fontSize: 11, color: 'var(--text-secondary)' }}>Попыток</div><div style={{ fontSize: 30, fontWeight: 700, color: '#0e7490' }}>{analytics.attempts.length}</div></div>
                  <div style={{ background: 'var(--bg-card)', boxShadow: 'var(--shadow-card)', borderRadius: 16, padding: 20, border: '1px solid rgba(19,122,57,0.25)' }}><div style={{ fontSize: 11, color: 'var(--text-secondary)' }}>Сдали</div><div style={{ fontSize: 30, fontWeight: 700, color: '#137a39' }}>{analytics.attempts.filter(x => x.is_passed).length}</div></div>
                </div>
                <div style={{ background: 'var(--bg-card)', boxShadow: 'var(--shadow-card)', borderRadius: 16, padding: 20, border: '1px solid var(--border-subtle)' }}>
                  <h4 style={{ fontSize: 14, fontWeight: 600, marginBottom: 12, color: 'var(--text-primary)' }}>Топ ошибок</h4>
                  {analytics.mistakes.length === 0 ? <p style={{ color: 'var(--text-muted)', fontSize: 12 }}>Ошибок пока нет</p> : analytics.mistakes.map((m, i) => (
                    <div key={i} style={{ padding: 10, borderRadius: 10, background: 'var(--bg-page)', marginBottom: 8, fontSize: 12, color: 'var(--text-secondary)' }}>
                      {m.text} <span style={{ color: '#dc2626', marginLeft: 8 }}>{Math.round(m.wrong / m.total * 100)}% ошибаются</span>
                    </div>
                  ))}
                </div>
                <div style={{ background: 'var(--bg-card)', boxShadow: 'var(--shadow-card)', borderRadius: 16, padding: 20, border: '1px solid var(--border-subtle)' }}>
                  <h4 style={{ fontSize: 14, fontWeight: 600, marginBottom: 12, color: 'var(--text-primary)' }}>Попытки сотрудников</h4>
                  {analytics.attempts.map(at => (
                    <div key={at.id} style={{ display: 'flex', justifyContent: 'space-between', padding: 10, borderRadius: 10, background: 'var(--bg-page)', marginBottom: 8, fontSize: 12 }}>
                      <span style={{ color: 'var(--text-primary)' }}>{[at.profiles?.first_name, at.profiles?.last_name].filter(Boolean).join(' ') || at.profiles?.email}</span>
                      <span style={{ color: at.is_passed ? '#137a39' : '#dc2626' }}>{at.score}% {at.is_passed ? '· сдал' : '· не сдал'}</span>
                    </div>
                  ))}
                </div>
              </>
