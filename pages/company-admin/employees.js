  ...
  return (
    <div className="theme-light" style={{ minHeight: '100vh', fontFamily: 'Inter, sans-serif', padding: '40px 32px' }}>
      <div style={{ maxWidth: 1600, margin: '0 auto' }}>
        ...
        {selectedIds.size > 0 && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 20px', marginBottom: 12, borderRadius: 14, background: 'rgba(234,88,12,0.06)', border: '1px solid var(--border-gold)' }}>
            ...
            <button onClick={applyBulkRole} disabled={!bulkRoleId || bulkApplying} style={{ ...ghostBtn, borderColor: 'var(--border-gold)', color: '#ea580c', opacity: !bulkRoleId ? 0.5 : 1 }} onMouseEnter={hoverOn} onMouseLeave={hoverOff}>{bulkApplying ? 'Применяем…' : 'Применить ко всем выбранным'}</button>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div>{PERMISSION_FIELDS.map(f => (
            <label key={f.key} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: 'var(--text-primary)', cursor: 'pointer', marginBottom: 6 }}>
              <input type="checkbox" checked={!!newEmployee[f.key]} onChange={e => setNewEmployee({ ...newEmployee, [f.key]: e.target.checked })} style={{ accentColor: '#ea580c' }} />{f.label}
            </label>
          ))}</div>
          <div style={{ display: 'flex', gap: 10 }}>
      ...
      <PremiumModal isOpen={!!editingEmployee} onClose={() => setEditingEmployee(null)} title="Редактировать сотрудника" showCloseButton={false}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div>{PERMISSION_FIELDS.map(f => (
            <label key={f.key} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: 'var(--text-primary)', cursor: 'pointer', marginBottom: 6 }}>
              <input type="checkbox" checked={!!editForm[f.key]} onChange={e => setEditForm({ ...editForm, [f.key]: e.target.checked })} style={{ accentColor: '#ea580c' }} />{f.label}
            </label>
          ))}</div>
          <div style={{ display: 'flex', gap: 10 }}>
