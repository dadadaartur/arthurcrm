  ...
  return (
    <div className="theme-light" style={{ minHeight: '100vh', fontFamily: 'Inter, sans-serif', padding: '40px 32px' }}>
      <div style={{ maxWidth: 1400, margin: '0 auto' }}>
        ...
        {data?.hint && (
          <div style={{ padding: '14px 18px', borderRadius: 14, background: 'rgba(234,88,12,0.06)', border: '1px solid var(--border-gold)', marginBottom: 20, fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.6 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px', borderBottom: '1px solid var(--border-subtle)' }}>
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                {[['completed', 'Выполнено'], ['rate', '% выполнения'], ['karma', 'Кармики']].map(([k, l]) => (
                  <button key={k} onClick={() => setSortBy(k)} style={{ fontSize: 11, padding: '4px 10px', borderRadius: 20, border: `1px solid ${sortBy === k ? 'var(--border-gold)' : 'var(--border-subtle)'}`, background: sortBy === k ? 'rgba(234,88,12,0.08)' : 'transparent', color: sortBy === k ? '#ea580c' : 'var(--text-secondary)', cursor: 'pointer' }}>{l}</button>
                ))}
              </div>
            </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ background: 'var(--bg-card)', boxShadow: 'var(--shadow-card)', borderRadius: 16, padding: 18, border: '1px solid var(--border-gold)' }}>
              ...
              {(data?.slowest || []).map(t => (
                <div key={t.task_id} style={{ display: 'flex', justifyContent: 'space-between', gap: 10, padding: '8px 0', borderTop: '1px solid var(--border-subtle)', fontSize: 12 }}>
                  <span style={{ color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{t.title}</span>
                  <span style={{ color: '#ea580c', fontWeight: 700, flexShrink: 0 }}>{fmtHours(t.avgHours)}</span>
                </div>
              ))}
