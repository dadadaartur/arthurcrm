...
function StatTile({ label, value, color, delay }) {
  return (
    <div style={{
      position: 'relative', padding: '22px 26px', borderRadius: 18,
...
function NavCard({ c, stats, delay, router }) {
  ...
  return (
    <div onClick={() => router.push(c.href)} className="nav-card"
      style={{
        '--card-color': c.color, position: 'relative', padding: c.primary ? 28 : 22, borderRadius: 20,
        background: c.primary ? 'linear-gradient(145deg, rgba(25,30,55,0.9), rgba(15,20,40,0.95))' : 'rgba(15,20,35,0.75)',
        backdropFilter: 'blur(14px)', border: `1px solid ${c.color}22`, cursor: 'pointer', overflow: 'hidden',
      ...
      <div className="nav-card-glow" style={{ position: 'absolute', top: -40, right: -40, width: 160, height: 160, background: `radial-gradient(circle, ${c.color}28, transparent 65%)`, filter: 'blur(24px)', pointerEvents: 'none', transition: 'transform 0.5s ease' }} />
      ...
      <div style={{ position: 'relative', zIndex: 2 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
          <div style={{ fontSize: c.primary ? 20 : 16, fontWeight: 600, color: '#fff', letterSpacing: 0.2 }}>{c.title}</div>
          {badgeVal > 0 && (
      ...
      <div style={{ position: 'relative', zIndex: 2, display: 'flex', alignItems: 'center', gap: 8, marginTop: 14 }}>
        <span className="nav-card-action" style={{ fontSize: 11, letterSpacing: 1.5, textTransform: 'uppercase', color: c.color, fontWeight: 600, transition: 'letter-spacing 0.35s' }}>Открыть</span>
        <span className="nav-card-arrow" style={{ display: 'inline-block', width: 14, height: 1, background: c.color, transition: 'width 0.35s, transform 0.35s' }} />
      </div>
      <style jsx>{`
        .nav-card:hover { transform: translateY(-4px); border-color: ${c.color}88; box-shadow: 0 14px 40px ${c.color}22, 0 0 0 1px ${c.color}44 inset; }
    ...
    <div style={{ minHeight: '100vh', background: '#000', color: '#fff', fontFamily: 'Inter, sans-serif', padding: '48px 40px' }}>
      <div style={{ maxWidth: 1600, margin: '0 auto' }}>
        <div style={{ marginBottom: 40 }}>
          ...
          <h1 style={{ fontSize: 40, fontWeight: 700, margin: 0, letterSpacing: -0.5, background: 'linear-gradient(135deg, #fff 0%, #a0e9ff 45%, #FFD700 85%, #ffb3c6 100%)', backgroundSize: '200% 200%', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', animation: 'titleShift 12s ease-in-out infinite alternate' }}>Управление компанией</h1>
        ...
        <div style={{ marginTop: 40, padding: '18px 24px', borderRadius: 16, background: 'linear-gradient(90deg, rgba(255,215,0,0.05), rgba(192,132,252,0.05), rgba(160,233,255,0.05))', border: '1px solid rgba(255,255,255,0.06)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
          ...
          <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.35)', letterSpacing: 1 }}>v2.2 · enterprise</div>
        </div>
      </div>
