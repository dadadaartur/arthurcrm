...
function CompanyAdminDashboard() {
  ...
  const [stats, setStats] = useState({ tasks: 0, employees: 0, goals: 0, karmaFund: 0 })
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const [tasksRes, employeesRes, goalsRes, karmaRes] = await Promise.all([
        ...
        supabase.from('company_karma_accounts').select('balance').eq('company_id', compId).maybeSingle()
      ])
      setStats({
        tasks: tasksRes.count || 0,
        employees: employeesRes.count || 0,
        goals: goalsRes.count || 0,
        karmaFund: karmaRes.data?.balance || 0
      })
      setLoading(false)
    }
    init()
    ...
    <div style={{ minHeight: '100vh', background: '#000', color: '#fff', fontFamily: 'Inter, sans-serif', padding: '40px 20px', position: 'relative' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, marginBottom: 32 }}>
          ...
          <Link href="/company-admin/karma" style={{ background: 'rgba(15,20,35,0.8)', backdropFilter: 'blur(10px)', borderRadius: 16, padding: 24, flex: '1 1 150px', textAlign: 'center', border: '1px solid rgba(255,215,0,0.35)', textDecoration: 'none' }}>
            <div style={{ fontSize: 32, fontWeight: 700, color: '#FFD700' }}>{stats.karmaFund}</div>
            <div style={{ fontSize: 14, color: '#aaa' }}>кармиков в казне</div>
          </Link>
        </div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12 }}>
          <Link href="/company-admin/karma" style={{ background: 'rgba(255,255,255,0.06)', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,215,0,0.25)', borderRadius: 50, padding: '10px 24px', color: '#fff', fontSize: 14, textDecoration: 'none' }}>Казна компании</Link>
