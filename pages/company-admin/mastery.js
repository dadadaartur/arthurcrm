...
const TYPE_HINTS = {
  cumulative: 'Сумма значений за период. Подходит для звонков, продаж, закрытых задач.',
  average: 'Среднее за период. Подходит для CSI, средней оценки, среднего чека.',
  ratio: 'Доля / конверсия: успешные ÷ всего × 100. Считается из двух параметров.',
  inverse: 'Меньше — лучше: время обработки, жалобы. Ультра = самое низкое значение.',
  plan: 'Процент выполнения плана: факт ÷ план × 100.',
  dynamics: 'Прирост к прошлому периоду в процентах.',
  binary: 'Доля случаев «да/нет»: соблюдение скрипта, подтверждение клиента.',
  min_period: 'Накопительно, но с обязательным минимумом в каждом дне/неделе.',
  rating: 'Место в команде (процентиль): обошёл 80% / 90% коллег.',
  weighted: 'Комплексный индекс из нескольких показателей с весами.',
}
...
function MasteryAdmin() {
  ...
  const load = async () => {
    ...
    if (r.ok) {
      const m = await r.json(); setMetrics(m)
      const p = {}; (m || []).forEach(x => (x.inputs || []).forEach(i => { p[i.key] = i })); setPool(Object.values(p))
    }
  ...
  const saveFill = async () => {
    ...
    if (res.ok) showSuccess(`Сохранено: ${entries.length} записей`); else showError('Ошибка сохранения')
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#000', color: '#fff', fontFamily: 'Inter, sans-serif', padding: '40px 32px' }}>
      <div style={{ maxWidth: 1600, margin: '0 auto' }}>
        <BackArrow href="/company-admin" title="Управление целями" extra={
          <div style={{ marginLeft: 'auto', display: 'flex', gap: 10, alignItems: 'center' }}>
            <span style={{ fontSize: 11, color: '#888', padding: '6px 14px', borderRadius: 20, border: '1px solid rgba(255,215,0,0.25)', background: 'rgba(255,215,0,0.06)' }}>Баланс компании: <b style={{ color: '#FFD700' }}>{companyKarma}</b> карм.</span>
          ...
          {metrics.map(m => (
            <div key={m.id} style={{ background: 'rgba(15,20,35,0.85)', borderRadius: 16, padding: 18, border: '1px solid rgba(255,255,255,0.08)', transition: 'border-color 0.25s', display: 'flex', flexDirection: 'column', gap: 12 }}
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 'auto', flexWrap: 'wrap' }}>
                <button onClick={() => setMaterialsMetric(m)} style={{ ...ghostBtn, padding: '6px 14px', fontSize: 11 }} onMouseEnter={hoverOn} onMouseLeave={hoverOff}>Материалы</button>
                <button onClick={() => setEditMetric(m)} style={{ ...ghostBtn, padding: '6px 14px', fontSize: 11, borderColor: 'rgba(160,233,255,0.4)', color: '#a0e9ff' }} onMouseEnter={hoverOn} onMouseLeave={hoverOff}>Редактировать</button>
        ...
        <div style={{ background: 'rgba(15,20,35,0.85)', borderRadius: 20, padding: 24, border: '1px solid rgba(255,255,255,0.08)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
            ...
            <div style={{ marginLeft: 'auto', display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              <button onClick={() => setManualOpen(o => !o)} style={ghostBtn} onMouseEnter={hoverOn} onMouseLeave={hoverOff}>{manualOpen ? 'Скрыть ввод' : 'Вручную'}</button>
            ...
            <div style={{ overflowX: 'auto' }}>
              <div style={{ minWidth: 200 + cols.length * 84 + formulaMetrics.length * 90 }}>
                <div style={{ display: 'grid', gridTemplateColumns: gridTemplate, gap: 8, marginBottom: 8 }}>
                  ...
                  {cols.map(c => <div key={c.key} style={{ textAlign: 'center', minWidth: 0 }}><div style={{ fontSize: 11, color: '#a0e9ff', fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={c.label}>{c.label}</div><div style={{ fontSize: 9, color: '#666' }}>{c.inverse ? 'меньше = лучше' : (c.unit || '')}</div></div>)}
                  {formulaMetrics.map(m => <div key={m.id} style={{ textAlign: 'center', minWidth: 0 }}><div style={{ fontSize: 11, color: '#FFD700', fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{m.name}</div><div style={{ fontSize: 9, color: '#666' }}>авто</div></div>)}
                </div>
                {employees.map(emp => (
                  <div key={emp.user_id} style={{ display: 'grid', gridTemplateColumns: gridTemplate, gap: 8, marginBottom: 8, alignItems: 'center' }}>
                    <div style={{ fontSize: 13, color: '#fff', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{empName(emp)}</div>
                    {cols.map(c => <input key={c.key} type="number" step="0.1" value={values[emp.user_id]?.[c.key] ?? ''} onChange={ev => setValues(v => ({ ...v, [emp.user_id]: { ...v[emp.user_id], [c.key]: ev.target.value } }))} className="input-field" style={{ width: '100%', textAlign: 'center', padding: '8px 4px' }} />)}
      ...
      {/* Создание / редактирование — одна широкая модалка */}
      <GoalFormModal open={createOpen || !!editMetric} initial={editMetric} pool={pool} setPool={setPool} companyKarma={companyKarma}
        onClose={() => { setCreateOpen(false); setEditMetric(null) }} onSave={saveMetric} />

      {/* Материалы — большая модалка */}
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(6px)', zIndex: 9998, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ width: 'min(980px, 95vw)', maxHeight: '88vh', overflowY: 'auto', background: 'linear-gradient(150deg, rgba(24,30,54,0.97), rgba(10,14,28,0.98))', border: '1px solid rgba(212,175,55,0.35)', borderRadius: 20, padding: 28, position: 'relative' }}>
        ...
        <div style={{ fontSize: 11, color: '#888', marginBottom: 6 }}>Баланс компании: <b style={{ color: '#FFD700' }}>{companyKarma}</b> карм. · Энергия начисляется автоматически (5/10/15/20)</div>
          ...
          <>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {Object.entries(TYPE_LABELS).map(([k, l]) => (
                <button key={k} onMouseEnter={() => setHover(k)} onMouseLeave={() => setHover(null)} onClick={() => { setForm(f => ({ ...f, kpi_type: k })); setStep('form') }}
                  style={{ padding: '10px 18px', borderRadius: 12, fontSize: 13, cursor: 'pointer', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.12)', color: '#ddd', transition: 'all 0.2s' }}
                  onMouseEnter={e => { e.currentTarget.style.borderColor = '#FFD700'; e.currentTarget.style.color = '#FFD700' }}
                  onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.12)'; e.currentTarget.style.color = '#ddd' }}>{l}</button>
              ))}
            </div>
            {hover && <div style={{ marginTop: 14, padding: 12, borderRadius: 12, background: 'rgba(160,233,255,0.07)', border: '1px solid rgba(160,233,255,0.3)', color: '#a0e9ff', fontSize: 12 }}>{TYPE_HINTS[hover]}</div>}
          </>
        ) : (
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
              ...
              {['thr_min', 'thr_mid', 'thr_top', 'thr_ultra'].map((k, i) => (
                <div key={k}><label style={{ fontSize: 11, color: BAND_COLORS[['min', 'mid', 'top', 'ultra'][i]], display: 'block', marginBottom: 4 }}>{BAND_LABELS[['min', 'mid', 'top', 'ultra'][i]]}</label><input type="number" step="0.1" className="input-field" style={{ width: '100%' }} value={form[k]} onChange={e => setForm({ ...form, [k]: e.target.value })} /></div>
              ))}
          ...
          {trainings.map(t => (
            <div key={t.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: 12, borderRadius: 12, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                {t.type === 'video' && <button onClick={() => setVideo(t)} style={{ ...ghostBtn, padding: '5px 12px', fontSize: 11 }} onMouseEnter={hoverOn} onMouseLeave={hoverOff}>Показать</button>}
                <Link href={`/company-admin/tests?training=${t.id}`} style={{ ...ghostBtn, padding: '5px 12px', fontSize: 11, textDecoration: 'none', borderColor: 'rgba(192,132,252,0.4)', color: '#c084fc' }}>Тест</Link>
              </div>
            </div>
          ))}
