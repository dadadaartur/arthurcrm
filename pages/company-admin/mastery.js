...
const pageBg = { minHeight: '100vh', color: '#fff', fontFamily: 'Inter, sans-serif', padding: '40px 32px', position: 'relative', background: 'radial-gradient(circle at 15% 15%, rgba(160,233,255,0.08), transparent 32%), radial-gradient(circle at 85% 20%, rgba(192,132,252,0.08), transparent 35%), radial-gradient(circle at 50% 100%, rgba(255,215,0,0.07), transparent 55%), #000' }
...
const fmtVal = (m, v) => (v == null || v === '') ? '—' : `${v}${unitWord(m)}`
...
function MasteryAdmin() {
  ...
  const stars = useMemo(() => {
    ...
    return Array.from({ length: 110 }, () => ({ left: Math.random() * 100, top: Math.random() * 100, size: Math.random() * 2 + 0.5, color: colors[Math.floor(Math.random() * colors.length)], op: Math.random() * 0.45 + 0.2, dur: Math.random() * 14 + 8, delay: Math.random() * 10 }))
  }, [])
  ...
  if (loading) return <div style={{ ...pageBg, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Spinner size={72} /></div>

  return (
    <div style={pageBg}>
      <div style={{ position: 'fixed', inset: 0, zIndex: 0, pointerEvents: 'none' }}>
        {stars.map((s, i) => (
          <div key={i} style={{ position: 'absolute', left: s.left + '%', top: s.top + '%', width: s.size, height: s.size, borderRadius: '50%', background: s.color, boxShadow: `0 0 ${s.size * 2}px ${s.color}`, opacity: s.op, animation: `mxTwinkle ${s.dur}s ease-in-out infinite`, animationDelay: s.delay + 's' }} />
        ))}
      </div>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', position: 'relative', zIndex: 1 }}>
        <BackArrow href="/company-admin" title="Управление целями" extra={
          <div style={{ marginLeft: 'auto', display: 'flex', gap: 12, alignItems: 'center' }}>
            <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.65)', textShadow: '0 0 10px rgba(255,215,0,0.3)' }}>Баланс: <b style={{ color: '#FFD700' }}>{companyKarma}</b> карм.</span>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '26px 44px', alignItems: 'start' }}>
          ...
          {metrics.map(m => (
            <div key={m.id} style={{ minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, flexWrap: 'wrap' }}>
                <span style={{ fontSize: 19, fontWeight: 600, lineHeight: 1.35, wordBreak: 'break-word', color: '#fff', textShadow: '0 0 18px rgba(255,255,255,0.22)' }}>{m.name}</span>
                <span style={{ display: 'inline-flex', gap: 6, flexShrink: 0, paddingTop: 2 }}>
                  <button className="mx-icon mx-edit" onClick={() => setEditMetric(m)} title="Редактировать"><PencilIcon /></button>
                  <button className="mx-icon mx-del" onClick={() => setDeleteTarget(m)} title="Удалить"><DeleteIcon /></button>
                </span>
              </div>

              <div style={{ marginTop: 5, fontSize: 12, color: '#c084fc', textShadow: '0 0 10px rgba(192,132,252,0.45)' }}>{TYPE_LABELS[m.kpi_type || 'cumulative']}</div>
              {m.kpi_type === 'inverse' && <div style={{ marginTop: 3, fontSize: 11, color: '#a0e9ff' }}>меньше значение = выше уровень</div>}
              ...
              <div style={{ marginTop: 14, position: 'relative', paddingLeft: 22 }}>
                ...
                {THRESH_ORDER.map(t => (
                  <div key={t.b} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '6px 0' }}>
                    <span style={{ width: 8, height: 8, borderRadius: '50%', background: BAND_COLORS[t.b], boxShadow: `0 0 10px ${BAND_COLORS[t.b]}`, marginLeft: -22, flexShrink: 0 }} />
                    {t.b === 'ultra'
                      ? <span className="mx-ultra" style={{ width: 74, fontSize: 13 }}>Ультра</span>
                      : <span style={{ width: 74, fontSize: 13, color: 'rgba(255,255,255,0.62)' }}>{t.label}</span>}
                    <span style={{ fontSize: 15, fontWeight: 600, color: BAND_COLORS[t.b], textShadow: `0 0 14px ${BAND_COLORS[t.b]}55` }}>{fmtVal(m, m[t.k])}</span>
                  </div>
                ))}
              </div>

              <button className="mx-related" onClick={() => setMaterialsMetric(m)}>связанные тесты и тренинги</button>
            </div>
          ))}
        </div>
      </div>
      ...
      <style jsx global>{`
        @keyframes mxTwinkle { 0%, 100% { opacity: 0.18; transform: scale(0.95); } 50% { opacity: 0.85; transform: scale(1.05); } }
        @keyframes mxSpin { to { transform: rotate(360deg); } }
        .mx-ring { transform-origin: 12px 12px; animation: mxSpin 7s linear infinite; }
        .mx-icon { width: 30px; height: 30px; border-radius: 10px; display: flex; align-items: center; justify-content: center; background: rgba(255,255,255,0.04); border: none; cursor: pointer; transition: all .35s cubic-bezier(.22,1,.36,1); }
        .mx-edit:hover { background: rgba(160,233,255,.12); box-shadow: 0 0 16px rgba(160,233,255,.35); transform: translateY(-1px) rotate(-8deg) scale(1.08); }
        .mx-del:hover { background: rgba(248,113,113,.12); box-shadow: 0 0 16px rgba(248,113,113,.35); transform: translateY(-1px) rotate(90deg) scale(1.08); }
        .mx-ultra { display: inline-block; background: linear-gradient(90deg, #c084fc, #FFD700, #ffb3c6, #a0e9ff, #c084fc); background-size: 300% 100%; -webkit-background-clip: text; background-clip: text; -webkit-text-fill-color: transparent; animation: mxUltraShift 4s linear infinite; font-weight: 700; }
        @keyframes mxUltraShift { 0% { background-position: 0% 50%; } 100% { background-position: 300% 50%; } }
        .mx-related { margin-top: 12px; background: none; border: none; padding: 0; cursor: pointer; font-size: 12px; letter-spacing: .3px; color: rgba(255,255,255,0.55); transition: color .25s; display: inline-block; }
        .mx-related:hover { color: #a0e9ff; text-shadow: 0 0 12px rgba(160,233,255,.55); }
        .mx-related::after { content: ''; display: block; height: 1px; margin-top: 4px; background: linear-gradient(90deg, #c084fc, #a0e9ff, #FFD700); background-size: 200% 100%; animation: mxLine 5s linear infinite; opacity: .35; }
        @keyframes mxLine { 0% { background-position: 0% 50%; } 100% { background-position: 200% 50%; } }
        .mx-close { position: absolute; top: 14px; right: 14px; width: 32px; height: 32px; background: none; border: none; cursor: pointer; color: #888; transition: transform .35s cubic-bezier(.22,1,.36,1), color .25s; z-index: 5; }
        .mx-close:hover { transform: rotate(90deg) scale(1.1); color: #f87171; }
      `}</style>
    </div>
  )
}
...
function GoalFormModal({ open, initial, pool, setPool, companyKarma, onClose, onSave }) {
  ...
  useEffect(() => {
    if (open) {
      setForm(initial ? {
        ...
        thr_ultra: initial.thr_ultra, thr_top: initial.thr_top, thr_mid: initial.thr_mid, thr_min: initial.thr_min,
        karma_ultra: initial.karma_ultra, karma_top: initial.karma_top, karma_mid: initial.karma_mid, karma_min: initial.karma_min,
        ...
        name: '', unit: '%', kpi_type: null, thr_ultra: 20, thr_top: 15, thr_mid: 10, thr_min: 5,
        karma_ultra: 10, karma_top: 5, karma_mid: 3, karma_min: 1, description: '', advice: '', num: '', den: '', mult: 100
      })
  ...
  const submit = () => {
    ...
    onSave({ name: form.name, unit: form.unit, kpi_type: form.kpi_type, description: form.description, advice: form.advice, mode: form.kpi_type === 'ratio' ? 'formula' : 'direct', ...nums, ...AUTO_ENERGY, inputs, formula }, initial?.id)
  }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(8px)', zIndex: 9998, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ width: 'min(980px, 95vw)', maxHeight: '88vh', overflowY: 'auto', background: 'linear-gradient(150deg, rgba(24,30,54,0.97), rgba(10,14,28,0.98))', border: '1px solid rgba(212,175,55,0.35)', borderRadius: 20, padding: 28, position: 'relative' }}>
        <button onClick={onClose} className="mx-close" title="Закрыть"><svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M3 3l10 10M13 3L3 13" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" /></svg></button>
        ...
        {step === 'type' ? (
              ...
              {TYPE_META.map((t, i) => {
                ...
                return (
                  <button key={t.key} onClick={() => { setForm(f => ({ ...f, kpi_type: t.key })); setStep('form') }}
                    style={{ display: 'grid', gridTemplateColumns: '52px 1fr', gap: 16, alignItems: 'center', textAlign: 'left', padding: '18px 20px', borderRadius: 18, cursor: 'pointer', border: 'none', background: `linear-gradient(135deg, ${t.color}${i < 2 ? '26' : i < 5 ? '18' : '10'}, transparent 72%)`, boxShadow: baseShadow, transition: 'all 0.25s ease' }}
                    onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = `0 0 ${Math.round(32 * intensity)}px ${t.color}44` }}
                    onMouseLeave={e => { e.currentTarget.style.transform = 'none'; e.currentTarget.style.boxShadow = baseShadow }}>
                    <div style={{ width: 44, height: 44, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: `${t.color}22`, color: t.color, fontWeight: 700, fontSize: 15, textShadow: `0 0 12px ${t.color}` }}>{i + 1}</div>
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                        <span style={{ color: '#fff', fontWeight: 600, fontSize: 15, textShadow: `0 0 14px ${t.color}66` }}>{t.label}</span>
                        {i < 2 && <span style={{ fontSize: 9, padding: '2px 8px', borderRadius: 10, background: `${t.color}22`, color: t.color, fontWeight: 700, letterSpacing: 0.6 }}>ПОПУЛЯРНЫЙ</span>}
                      </div>
                      <div style={{ color: 'rgba(255,255,255,0.62)', fontSize: 12, marginTop: 4, lineHeight: 1.5 }}>{t.hint}</div>
                      <div style={{ color: t.color, opacity: 0.75, fontSize: 11, marginTop: 4 }}>Примеры: {t.ex}</div>
                    </div>
                  </button>
                )
              })}
          ...
          <>
            <h3 style={{ fontSize: 18, fontWeight: 600, margin: '0 0 18px', color: '#fff' }}>{initial ? 'Редактировать цель' : 'Новая цель'} · {TYPE_LABELS[form.kpi_type]}</h3>
            {isInverse && <div style={{ marginBottom: 16, padding: 12, borderRadius: 12, background: 'rgba(160,233,255,0.07)', border: '1px solid rgba(160,233,255,0.3)', color: '#a0e9ff', fontSize: 12 }}>Меньше — лучше. Визуально уровни всегда идут сверху вниз: Ультра, Топ, Средн, Мин.</div>}
            ...
            <div style={{ marginTop: 22 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14 }}>
                {THRESH_ORDER.map(t => (
                  <div key={t.k}><label style={{ fontSize: 11, display: 'block', marginBottom: 6, color: BAND_COLORS[t.b], fontWeight: 600 }}>{t.label}</label><input type="number" step="0.1" className="input-field" style={{ width: '100%' }} value={form[t.k]} onChange={e => setForm({ ...form, [t.k]: e.target.value })} /></div>
                ))}
              </div>
            </div>
            <div style={{ marginTop: 18 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14 }}>
                {KARMA_ORDER.map(x => (
                  <div key={x.k}><label style={{ fontSize: 11, display: 'block', marginBottom: 6, color: BAND_COLORS[x.b], fontWeight: 600 }}>{BAND_LABELS[x.b]}</label><input type="number" className="input-field" style={{ width: '100%' }} value={form[x.k]} onChange={e => setForm({ ...form, [x.k]: e.target.value })} /></div>
                ))}
              </div>
            </div>
...
function MaterialsModal({ metric, onClose }) {
  ...
  const add = async () => {
    ...
    const h = await auth()
    const r = await fetch('/api/company-admin/kpi/trainings', { method: 'POST', headers: { ...h, 'Content-Type': 'application/json' }, body: JSON.stringify({ metric_id: metric.id, title: form.title, type: form.type, url: form.url || null, content: form.content || null, recommend_below: 'all' }) })
    if (r.ok) { showSuccess('Тренинг добавлен'); setForm({ title: '', type: 'video', url: '', content: '' }); load(metric.id) }
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(6px)', zIndex: 9998, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ width: 'min(880px, 94vw)', maxHeight: '88vh', overflowY: 'auto', background: 'linear-gradient(150deg, rgba(24,30,54,0.97), rgba(10,14,28,0.98))', border: '1px solid rgba(212,175,55,0.35)', borderRadius: 20, padding: 28, position: 'relative' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 }}>
          {trainings.map(t => (
            <div key={t.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: 12, borderRadius: 12, background: 'rgba(255,255,255,0.03)' }}>
              <div>
                <div style={{ fontSize: 13, color: '#fff' }}>{t.title}</div>
                <div style={{ fontSize: 11, color: '#888', marginTop: 2 }}>{t.type === 'video' ? 'Видео' : t.type === 'text' ? 'Текст' : t.type}</div>
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                {(t.url || t.video_path) && <button onClick={() => setVideo(t)} style={ghostBtn} onMouseEnter={hoverOn} onMouseLeave={hoverOff}>Смотреть</button>}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          ...
          <select className="input-field" value={form.type} onChange={e => setForm({ ...form, type: e.target.value })}><option value="video">Видео</option><option value="text">Текст</option></select>
          {form.type === 'video' && <input className="input-field" style={{ gridColumn: '1 / -1' }} placeholder="URL видео" value={form.url} onChange={e => setForm({ ...form, url: e.target.value })} />}
          {form.type === 'text' && <textarea className="input-field" style={{ gridColumn: '1 / -1' }} rows={3} placeholder="Текст тренинга" value={form.content} onChange={e => setForm({ ...form, content: e.target.value })} />}
          <div style={{ gridColumn: '1 / -1' }}><button onClick={add} style={ghostBtn} onMouseEnter={hoverOn} onMouseLeave={hoverOff}>Добавить тренинг</button></div>
        </div>
      </div>
