      ...
      <div style={{ maxWidth: 1600, margin: '0 auto' }}>
        <BackArrow href="/company-admin" title="Обучение: тренинги и тесты" extra={
          <div style={{ marginLeft: 'auto', display: 'flex', gap: 8 }}>
            <button onClick={() => setTab('active')} style={{ ...ghostBtn, color: tab === 'active' ? '#ea580c' : 'var(--text-primary)', borderColor: tab === 'active' ? 'var(--border-gold)' : 'var(--border-gold)' }}>Активные</button>
            <button onClick={() => setTab('archive')} style={{ ...ghostBtn, color: tab === 'archive' ? '#ea580c' : 'var(--text-primary)', borderColor: tab === 'archive' ? 'var(--border-gold)' : 'var(--border-gold)' }}>Архив</button>
                ...
                <div style={{ marginTop: 6, display: 'flex', flexDirection: 'column', gap: 6 }}>
                  ...
                  {watchStats(t).map(s => (
                    <div key={s.uid} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11 }}>
                      ...
                      <div style={{ width: 90, height: 5, borderRadius: 3, background: 'var(--bg-page)' }}><div style={{ height: '100%', width: s.pct + '%', borderRadius: 3, background: s.pct >= 90 ? '#137a39' : '#ea580c' }} /></div>
                      <span style={{ color: s.pct >= 90 ? '#137a39' : '#ea580c', width: 34, textAlign: 'right' }}>{s.pct}%</span>
                    </div>
                  ))}
                </div>
              )}
