        ...
        <div style={{ position: 'absolute', zIndex: 30, marginTop: 4, width: '100%', maxHeight: 200, overflowY: 'auto', borderRadius: 12, border: '1px solid var(--border-subtle)', background: 'var(--bg-card)', boxShadow: 'var(--shadow-card-hover)' }}>
          {employees.filter(e => empName(e).toLowerCase().includes(search.toLowerCase())).map(e => (
            <button type="button" key={e.user_id} onMouseDown={() => { onChange(e.user_id); setSearch('') }}
              ...
              <span style={{ fontSize: 12, color: 'var(--text-primary)' }}>{empName(e)}</span>
            </button>
          ))}
        </div>
      )}
      ...
      <div key={node.id}>
        ...
        >
          ...
          <div style={{ display: 'flex', gap: 6 }} onClick={e => e.stopPropagation()}>
            ...
            <button onClick={() => setEditDept(node)} title="Редактировать" style={{ background: 'rgba(14,116,144,0.06)', border: '1px solid rgba(14,116,144,0.3)', borderRadius: 10, padding: '6px 9px', color: '#0e7490', cursor: 'pointer' }}>
    ...
    <div className="theme-light" style={{ minHeight: '100vh', fontFamily: 'Inter, sans-serif', padding: '40px 32px' }}>
      <div style={{ maxWidth: 1600, margin: '0 auto' }}>
        <BackArrow href="/company-admin" title="Отделы компании" extra={
          <button onClick={() => setEditDept({ parent_department_id: null, name: '', manager_user_id: null })} style={{ ...ghostBtn, marginLeft: 'auto', borderColor: 'var(--border-gold)', color: 'var(--accent-gold)' }} onMouseEnter={hoverOn} onMouseLeave={hoverOff}>+ Новый отдел</button>
        } />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: selectedDept ? '1fr 380px' : '1fr', gap: 24, alignItems: 'start' }}>
            ...
            {unassigned.length > 0 && (
              <div style={{ marginTop: 20, padding: 16, borderRadius: 14, background: 'rgba(220,38,38,0.04)', border: '1px solid rgba(220,38,38,0.2)' }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                  {unassigned.map(e => <span key={e.user_id} style={{ fontSize: 12, color: 'var(--text-primary)', padding: '4px 10px', borderRadius: 20, background: 'var(--bg-page)' }}>{empName(e)}</span>)}
                </div>
              </div>
            )}
          </div>
          ...
          {selectedDept && (
            <div style={{ background: 'var(--bg-card)', boxShadow: 'var(--shadow-card-hover)', borderRadius: 20, padding: 22, border: '1px solid var(--border-gold)', position: 'sticky', top: 20 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
                <h3 style={{ fontSize: 16, fontWeight: 600, margin: 0, color: 'var(--text-primary)' }}>{selectedDept.name}</h3>
              ...
              <label style={{ fontSize: 11, color: 'var(--text-secondary)', display: 'block', marginBottom: 6 }}>Руководитель отдела</label>
              ...
              <label style={{ fontSize: 11, color: 'var(--text-secondary)', display: 'block', marginBottom: 6 }}>Сотрудники отдела ({countEmployees(selectedDept.id)})</label>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 14, maxHeight: 260, overflowY: 'auto' }}>
                {employees.filter(e => e.department_id === selectedDept.id).map(e => (
                  <div key={e.user_id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 10px', borderRadius: 10, background: 'var(--bg-page)' }}>
                    <span style={{ fontSize: 12, color: 'var(--text-primary)', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{empName(e)}</span>
                    <button onClick={() => assignEmployee(e.user_id, { departmentId: null })} title="Убрать из отдела" style={{ background: 'none', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer' }}>
                      <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M18 6L6 18M6 6l12 12" /></svg>
                    </button>
                  </div>
                ))}
              </div>
              <label style={{ fontSize: 11, color: 'var(--text-secondary)', display: 'block', marginBottom: 6 }}>Добавить сотрудника в отдел</label>
      ...
      {editDept && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.5)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: 20 }} onClick={() => setEditDept(null)}>
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border-subtle)', boxShadow: 'var(--shadow-card-hover)', borderRadius: 20, padding: 26, maxWidth: 420, width: '94%' }} onClick={e => e.stopPropagation()}>
            <h3 style={{ fontSize: 17, fontWeight: 600, margin: '0 0 16px', color: 'var(--text-primary)' }}>{editDept.id ? 'Редактировать отдел' : 'Новый отдел'}</h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div>
                <label style={{ fontSize: 12, color: 'var(--text-secondary)', display: 'block', marginBottom: 6 }}>Родительский отдел</label>
              ...
              <div>
                <label style={{ fontSize: 12, color: 'var(--text-secondary)', display: 'block', marginBottom: 6 }}>Руководитель</label>
                {editDept.id ? (
                  ...
                  <>
                    <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
                      ...
                      <Seg active={editDept.inviteByEmail} onClick={() => setEditDept({ ...editDept, inviteByEmail: true, manager_user_id: null })} color="#137a39">Пригласить по email</Seg>
                    </div>
                    ...
                    {editDept.inviteByEmail && <p style={{ fontSize: 11, color: 'var(--text-secondary)', margin: '6px 0 0' }}>Если этого человека ещё нет в компании — он получит приглашение и автоматически станет руководителем этого отдела, как только зарегистрируется.</p>}
                  </>
                )}
              </div>
              <div style={{ display: 'flex', gap: 10, marginTop: 6 }}>
                <button onClick={() => setEditDept(null)} className="btn-outline" style={{ flex: 1 }}>Отмена</button>
                <button onClick={saveDept} style={{ ...ghostBtn, flex: 1, borderColor: 'var(--border-gold)', color: 'var(--accent-gold)' }} onMouseEnter={hoverOn} onMouseLeave={hoverOff}>Сохранить</button>
              </div>
            </div>
          </div>
        </div>
