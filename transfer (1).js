...
export default async function handler(req, res) {
  ...
  const { recipientUserId, recipientEmail, amount, comment } = req.body
  const transferAmount = Number.isInteger(amount) ? amount : parseInt(amount, 10)

  if ((!recipientUserId && !recipientEmail) || !Number.isInteger(transferAmount) || transferAmount <= 0) {
  ...
  // Получатель выбирается из выпадающего списка коллег (recipientUserId) —
  // это основной путь теперь. recipientEmail оставлен как fallback для
  // старых клиентов/интеграций, которые ещё шлют email напрямую.
  let recipient
  if (recipientUserId) {
    const { data, error: recipError } = await supabaseAdmin
      .from('profiles')
  ...
  const recipientLabel = recipient.email || recipientUserId
  const { error: notifyError } = await supabaseAdmin.from('notifications').insert([
    {
      user_id: ctx.user.id,
      message: `Вы перевели ${transferAmount} кармиков → ${recipientLabel}${comment ? '. ' + comment : ''}`,
      link: '/history'
    },
    {
