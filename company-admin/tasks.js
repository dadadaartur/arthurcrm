      ...
      <div style={{ maxWidth: 1600, margin: '0 auto' }}>
        <BackArrow href="/company-admin" title="Управление заданиями" extra={
          <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 14 }}>
            <Link href="/company-admin/tasks-analytics" style={{ fontSize: 12, color: '#ea580c', textDecoration: 'none', fontWeight: 600 }}>Аналитика →</Link>
            ...
            <form onSubmit={handleCreateTask}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
                ...
                <div style={{ gridColumn: 'span 2' }}>
                  ...
                  {form.target_role !== 'specific' && (
                    <div style={{ marginTop: 8, padding: '8px 12px', borderRadius: 10, background: 'rgba(234,88,12,0.06)', border: '1px solid var(--border-gold)', fontSize: 12 }}>
                      {audienceLoading ? (
                ...
                <div style={{ gridColumn: 'span 3' }}>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    ...
                    <Seg active={form.visual_tier === 'premium'} onClick={() => setForm({ ...form, visual_tier: 'premium' })} color="#ea580c">Премиум — золотой бейдж</Seg>
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', gap: 16, alignItems: 'center', flexWrap: 'wrap' }}>
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: 'var(--text-primary)', cursor: 'pointer' }}>
                    <input type="checkbox" checked={form.requires_review} onChange={e => setForm({ ...form, requires_review: e.target.checked })} style={{ accentColor: '#ea580c' }} />
                    Требуется проверка
                  </label>
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: 'var(--text-primary)', cursor: 'pointer' }}>
                    <input type="checkbox" checked={form.requires_proof} onChange={e => setForm({ ...form, requires_proof: e.target.checked })} style={{ accentColor: '#ea580c' }} />
                    Медиа-подтверждение
                  </label>
                  {form.requires_proof && (
            ...
            {pendingReviews.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 32 }}>
                {pendingReviews.map(item => (
                  <div key={item.id} style={{ background: 'var(--bg-card)', boxShadow: 'var(--shadow-card)', borderRadius: 16, padding: 18, border: '1px solid rgba(234,88,12,0.35)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: 14 }}>
            ...
            {tasks.map(t => (
              <div key={t.id} style={{ background: 'var(--bg-card)', boxShadow: 'var(--shadow-card)', borderRadius: 16, padding: 18, border: '1px solid var(--border-subtle)', transition: 'border-color 0.25s' }}
                ...
                <span style={{ display: 'inline-block', fontSize: 9, padding: '1px 8px', borderRadius: 20, marginTop: 6, background: t.department_id ? 'rgba(14,116,144,0.08)' : 'rgba(234,88,12,0.08)', color: t.department_id ? '#0e7490' : '#ea580c', border: `1px solid ${t.department_id ? 'rgba(14,116,144,0.3)' : 'var(--border-gold)'}` }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.5)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: 20 }} onClick={() => !reviewLoading && setSelectedReview(null)}>
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border-subtle)', boxShadow: 'var(--shadow-card-hover)', borderRadius: 20, padding: 26, maxWidth: 480, width: '94%', maxHeight: '85vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
              ...
              <button onClick={() => handleReview('approve')} disabled={reviewLoading} style={{ padding: '10px 14px', borderRadius: 12, fontSize: 13, fontWeight: 600, background: 'linear-gradient(135deg, rgba(138,98,8,0.15), rgba(19,122,57,0.12))', border: '1px solid var(--border-gold)', color: '#ea580c', cursor: 'pointer' }}>
