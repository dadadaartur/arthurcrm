        ...
        <div style={{ position: 'absolute', zIndex: 30, marginTop: 4, width: '100%', maxHeight: 200, overflowY: 'auto', borderRadius: 12, border: '1px solid var(--border-subtle)', background: 'var(--bg-card)', boxShadow: 'var(--shadow-card-hover)' }}>
          {employees.filter(e => empName(e).toLowerCase().includes(search.toLowerCase())).map(e => (
            <button type="button" key={e.user_id} onMouseDown={() => { onChange(e.user_id); setSearch('') }}
              ...
              <div style={{ width: 20, height: 20, borderRadius: '50%', background: 'rgba(234,88,12,0.1)', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, color: '#ea580c', overflow: 'hidden' }}>
...
function RoleBadge({ roleName }) {
  ...
  return (
    <span style={{ fontSize: 9.5, fontWeight: 600, padding: '2px 8px', borderRadius: 20, flexShrink: 0, background: isAdmin ? 'rgba(234,88,12,0.12)' : 'rgba(14,116,144,0.1)', color: isAdmin ? '#ea580c' : '#0e7490', border: `1px solid ${isAdmin ? 'var(--border-gold)' : 'rgba(14,116,144,0.3)'}` }}>
      {roleName}
    </span>
  )
}
  ...
  if (creating) {
    return (
      <div style={{ display: 'flex', gap: 6 }}>
        ...
        }} style={{ ...ghostBtn, padding: '8px 14px', borderColor: 'var(--border-gold)', color: '#ea580c' }}>{saving ? '…' : 'Создать'}</button>
    ...
    return (
      <div key={node.id}>
        <div
          ...
          style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', marginLeft: depth * 28, borderRadius: 14, cursor: 'pointer', background: selectedDept?.id === node.id ? 'rgba(234,88,12,0.06)' : 'var(--bg-card)', boxShadow: 'var(--shadow-card)', border: `1px solid ${selectedDept?.id === node.id ? 'var(--border-gold)' : 'var(--border-subtle)'}`, marginBottom: 8, transition: 'all 0.2s' }}
        >
