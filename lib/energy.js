...
export async function creditEnergy(a, userId, amount, source = null, description = null) {
  ...
  // Журнал — без него посчитать «сколько энергии заработано именно за
  // этот период» для гонки/кубка физически невозможно, kpi_energy
  // хранит только текущий накопленный итог, не историю (добавлено
  // 6 сентября 2026 вместе с переходом рейтингов на энергию).
  await a.from('energy_transactions').insert({ user_id: userId, amount: allowed, source, description })
