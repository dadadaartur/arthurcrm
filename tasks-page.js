...
export default function TasksPage() {
  ...
  const handleStart = async (assignmentId) => {
    // .eq('status', 'assigned') — defense in depth: основную защиту от
    // недопустимых переходов статуса даёт триггер БД
    // (trg_enforce_task_assignment_transition), но так запрос сразу же
    // не находит строку и не долетает до триггера, если задание уже не
    // в статусе "assigned" (например, двойной клик или устаревшие данные
    // на экране).
