...
// Страница Кармического Центробанка — только для основателя
// (arturgalkin.ru@mail.ru). Регулятор эмиссии, просмотр всех компаний,
// ручная эмиссия по тарифу для теста.
    ...
    <div className="w-full px-8 py-10">
      <div className="flex items-center gap-4 mb-8">
        <div
          ...
          className="w-10 h-10 rounded-full flex items-center justify-center cursor-pointer hover:bg-white/5 transition-colors"
        ...
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Регулятор эмиссии кармиков. Доступ: только основатель</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        ...
        <div className="premium-card">
          <div className="text-xs text-gray-400 uppercase tracking-wider mb-2">Отработано (списано)</div>
          <div className="text-3xl font-bold" style={{ color: '#a0e9ff' }}>{bank?.total_utilized ?? 0}</div>
          <div className="text-xs text-gray-500 mt-1">ушло в магазин</div>
        </div>
      </div>
          ...
          <table className="w-full text-left text-sm">
            <thead className="text-gray-400 border-b border-gray-700">
              <tr>
                ...
                <th className="py-3 pr-4 font-medium">Сотрудников</th>
            ...
            <tbody>
              {companies.map(c => {
                ...
                const expectedEmission = tariff ? tariff.karma_per_employee * Math.max(c.employee_count, 1) : 0
                const validUntil = c.valid_until ? new Date(c.valid_until).toLocaleDateString('ru') : '—'
                return (
                  <tr key={c.id} className="border-b border-gray-800 hover:bg-white/[0.02]">
                    ...
                    <td className="py-3 pr-4">
                      <span className="font-semibold" style={{ color: '#FFD700' }}>{c.balance ?? 0}</span>
                      <span className="text-xs text-gray-500 ml-1">кармиков</span>
                    </td>
                    <td className="py-3 pr-4 text-gray-300">
                      {tariff ? `+${expectedEmission}` : '—'}
                    </td>
                    <td className="py-3 pr-4 text-gray-400">{validUntil}</td>
      ...
      {/* История эмиссий */}
      <div className="premium-card">
        <h2 className="text-xl font-semibold mb-4 text-white">Журнал эмиссий</h2>
        {ledger.length === 0 ? (
          <p className="text-gray-500 text-sm">Эмиссий ещё не было</p>
        ) : (
          <div className="space-y-2 max-h-96 overflow-y-auto">
            {ledger.map(e => (
              <div key={e.id} className="flex justify-between items-center p-3 rounded-lg bg-gray-800/50">
                ...
                <span className="font-semibold" style={{ color: '#4ade80' }}>+{e.amount}</span>
              </div>
            ))}
          </div>
        )}
...
CentralBank.bypassLayout = false
