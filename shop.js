...
export default function Shop() {
  ...
  return (
    <div className="theme-light" style={{ width: '100vw', minHeight: '100vh', overflow: 'hidden', position: 'relative', fontFamily: 'Inter, sans-serif' }}>
        ...
        <div style={{ maxWidth: '1400px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32, flexWrap: 'wrap', gap: 16 }}>
            ...
            <div style={{
              ...
              <span style={{ fontSize: 14, color: 'var(--text-secondary)', fontWeight: 400 }}>Баланс</span>
              <span style={{
                fontSize: 20,
                fontWeight: 600,
              ...
              <span style={{ fontSize: 13, color: 'var(--accent-gold)', fontWeight: 400 }}>кармиков</span>
            </div>
          </div>
          ...
          {featuredRewards.length > 0 && (
            <div style={{ marginBottom: 28 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#ea580c" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M13 2L3 14h7l-1 8 11-14h-7l1-6z" /></svg>
              ...
              <div style={{ display: 'flex', gap: 16, overflowX: 'auto', paddingBottom: 10, scrollSnapType: 'x mandatory' }}>
                {featuredRewards.map(reward => (
                  <div key={reward.id} onClick={() => setSelectedReward(reward)} style={{
                    ...
                    boxShadow: 'var(--shadow-card)', transition: 'transform 0.25s'
                  }} onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-4px)' }} onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)' }}>
                    <div style={{ height: 120, position: 'relative' }}>
                      {reward.image_url ? <img src={reward.image_url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <div style={{ width: '100%', height: '100%', background: 'var(--bg-page)' }} />}
                    ...
                    <div style={{ padding: 14 }}>
                      <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-primary)', marginBottom: 6, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{reward.name}</div>
                      <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--accent-gold)' }}>{reward.cost} {getKarmikWord(reward.cost)}</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {visibleRewards.length === 0 ? (
            <div style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '60px 0' }}>В этой категории пока нет товаров</div>
          ) : (
          <div style={{
            display: 'grid',
            ...
            {visibleRewards.map(reward => {
              ...
              return (
                <div key={reward.id} style={{
                  ...
                  borderRadius: 24,
                  overflow: 'hidden',
                  border: isLocked ? '1px solid rgba(124,58,237,0.35)' : '1px solid var(--border-subtle)',
                  boxShadow: 'var(--shadow-card)',
                ...
                >
                  <div style={{ width: '100%', height: 220, position: 'relative', overflow: 'hidden' }}>
                    ...
                    <span style={{ position: 'absolute', top: 12, right: 12, background: 'rgba(255,255,255,0.92)', color: 'var(--accent-gold)', padding: '4px 10px', borderRadius: 20, fontSize: 12, fontWeight: 600 }}>
                      {typeLabels[reward.type] || 'Товар'}
                    </span>
                    {reward.requires_approval && (
                      <span style={{ position: 'absolute', top: 12, left: 12, background: 'rgba(220,38,38,0.92)', color: '#fff', padding: '4px 10px', borderRadius: 20, fontSize: 12, fontWeight: 600 }}>Требуется согласование</span>
                    )}
                    {isLocked && (
                      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8, background: 'rgba(255,255,255,0.55)' }}>
                        <span style={{ width: 44, height: 44, borderRadius: '50%', background: 'rgba(124,58,237,0.1)', border: '1px solid rgba(124,58,237,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#7c3aed" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="5" y="11" width="14" height="10" rx="2" /><path d="M8 11V7a4 4 0 0 1 8 0v4" /></svg>
                        </span>
                        <span style={{ fontSize: 11, color: '#6d28d9', fontWeight: 600, textAlign: 'center', padding: '0 16px' }}>Откроется после партнёрского задания</span>
                      </div>
                    )}
                  </div>
                  <div style={{ padding: '20px 24px 24px', flex: 1, display: 'flex', flexDirection: 'column' }}>
                    <h3 style={{ fontSize: 20, fontWeight: 600, color: 'var(--text-primary)', marginBottom: 10 }}>{reward.name}</h3>
                    <p style={{ fontSize: 14, color: 'var(--text-secondary)', marginBottom: 20, lineHeight: 1.6, flex: 1, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical' }}>{reward.description}</p>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 'auto' }}>
                      <span style={{ fontSize: 18, fontWeight: 600, color: 'var(--accent-gold)' }}>{reward.cost} {word}</span>
                      {isLocked ? (
                        <span style={{ fontSize: 12, color: '#7c3aed', padding: '10px 18px', border: '1px solid rgba(124,58,237,0.4)', borderRadius: 14 }}>Заблокировано</span>
                      ) : limitReached ? (
                        <span style={{ fontSize: 12, color: '#137a39', padding: '10px 18px', border: '1px solid rgba(19,122,57,0.35)', borderRadius: 14, display: 'flex', alignItems: 'center', gap: 6 }}>
                      ...
                      <button
                        ...
                        disabled={loading}
                        style={{
                          background: 'linear-gradient(135deg, rgba(138,98,8,0.14), rgba(124,58,237,0.1), rgba(14,116,144,0.1))',
                          border: '1px solid var(--border-gold)',
                          borderRadius: 14,
                          padding: '10px 24px',
                          fontSize: 14,
            ...
            <div style={{ display: 'flex', gap: 40, flexWrap: 'wrap' }}>
              ...
              <div style={{ flex: '1 1 300px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                ...
                <h2 style={{
                  ...
                  fontWeight: 700,
                  marginBottom: 16,
                  background: 'linear-gradient(135deg, #0e7490, #db2777)',
                  WebkitBackgroundClip: 'text',
                  WebkitTextFillColor: 'transparent'
                }}>
                  {selectedReward.name}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginTop: 'auto' }}>
                  ...
                  {selectedReward.requires_unlock && !unlockedKeys.includes(selectedReward.requires_unlock) ? (
                    <span style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 14, color: '#7c3aed', padding: '14px 24px', border: '1px solid rgba(124,58,237,0.4)', borderRadius: 14 }}>
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#7c3aed" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="5" y="11" width="14" height="10" rx="2" /><path d="M8 11V7a4 4 0 0 1 8 0v4" /></svg>
                    ...
                    <span style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 14, color: '#137a39', padding: '14px 24px', border: '1px solid rgba(19,122,57,0.35)', borderRadius: 14 }}>
      ...
      <PremiumModal isOpen={modal.show} onClose={() => setModal({ ...modal, show: false })} title={modal.type === 'success' ? 'Успешно' : 'Ошибка'}>
        <p style={{ color: 'var(--text-primary)' }}>{modal.message}</p>
      </PremiumModal>
